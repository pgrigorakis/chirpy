...
const getAllChirpsByCreateDate = `-- name: GetAllChirpsByCreateDate :many
SELECT id, created_at, updated_at, body, user_id FROM chirps
ORDER BY created_at
`

func (q *Queries) GetAllChirpsByCreateDate(ctx context.Context) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAllChirpsByCreateDate)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
